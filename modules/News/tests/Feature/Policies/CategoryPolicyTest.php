...
it('allows any user to view any categories', function () {
    $user = auth()->user();

    expect($user->can('viewAny', Category::class))->toBeTrue();
});

it('allows any user to view a category', function () {
    $category = Category::factory()->create();

    expect(auth()->user()->can('view', $category))->toBeTrue();
});

it('allows an administrator to create a category', function () {
    auth()->user()->update(['is_administrator' => true]);

    expect(auth()->user()->can('create', Category::class))->toBeTrue();
});

it('allows a user with ROLE_NEWS_ADMIN to create a category', function () {
...
it('denies a regular user to create a category', function () {
    expect(auth()->user()->can('create', Category::class))->toBeFalse();
});

it('allows an administrator to update a category', function () {
    auth()->user()->update(['is_administrator' => true]);
    $category = Category::factory()->create();
...
it('denies a regular user to update a category', function () {
    $category = Category::factory()->create();

    expect(auth()->user()->can('update', $category))->toBeFalse();
});

it('allows an administrator to delete a category', function () {
    auth()->user()->update(['is_administrator' => true]);
    $category = Category::factory()->create();
...
it('denies a regular user to delete a category', function () {
    $category = Category::factory()->create();
