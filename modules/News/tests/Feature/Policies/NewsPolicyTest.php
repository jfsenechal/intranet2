...
it('allows any user to view any news', function () {
    expect(auth()->user()->can('viewAny', News::class))->toBeTrue();
});

it('allows any user to view a news', function () {
    $news = News::factory()->create();

    expect(auth()->user()->can('view', $news))->toBeTrue();
});

it('allows any user to create news', function () {
    expect(auth()->user()->can('create', News::class))->toBeTrue();
});

it('allows an administrator to update news', function () {
...
it('allows a user with ROLE_NEWS_ADMIN to update news', function () {
...
it('allows the author to update their own news', function () {
    $user = auth()->user();
...
it('denies a regular user to update news they did not create', function () {
    $news = News::factory()->create();
...
it('allows an administrator to delete news', function () {
    auth()->user()->update(['is_administrator' => true]);
    $news = News::factory()->create();
...
it('allows the author to delete their own news', function () {
    $user = auth()->user();
    $news = News::factory()->create(['user_add' => $user->username]);
...
it('denies a regular user to delete news they did not create', function () {
    $news = News::factory()->create();
    $news->updateQuietly(['user_add' => 'other-user']);
